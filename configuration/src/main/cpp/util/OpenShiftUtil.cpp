...
const ::std::string OpenShiftUtil::FILE_SEPARATOR = "/";
...
::std::string OpenShiftUtil::getConfigurationDir() {
    ...
    if (dirFromEnv.find_last_of(FILE_SEPARATOR) == (dirFromEnv.length() - 1)) {
        return dirFromEnv + openShiftConfigDir;
    } else {
        return dirFromEnv + FILE_SEPARATOR + openShiftConfigDir;
    }
}
...
::std::string OpenShiftUtil::getRepoPath(const ::std::string& file) {
    if (file.find(FILE_SEPARATOR) == 0) {
        //absolute path
        return file;
    }

    return getRepoDir() + FILE_SEPARATOR + file;
}
